...
def get_account_info(attributes, remote_app):
    ...
    external_id = attributes[mappings['eduPersonPrincipalName']][0]
    #full_name = attributes[mappings['full_name']][0]

    sn = attributes[mappings['sn']][0]
    givenName = attributes[mappings['givenName']][0]
    #eduPersonPrincipalName = attributes[mappings['eduPersonPrincipalName']][0]
    external_id = attributes[mappings['eduPersonPrincipalName']][0]
    mail = attributes[mappings['mail']][0]

    return dict(
        user=dict(
            email=mail,
            profile=dict(full_name=sn, username=slugify(sn)),
        ),
